...
/// Returns a new artifact identifier whose date and millisecond components are
/// sortable while the suffix prevents same-tick collisions across processes.
/// The random suffix is generated by the operating system CSPRNG.
pub fn artifact_instance_id(date: &str) -> Result<String> {
    let date = date
        .chars()
        .filter(|character| character.is_ascii_digit())
        .collect::<String>();
    if date.is_empty() {
        bail!("Artifact date cannot be empty");
    }
    ...
    getrandom::getrandom(&mut random)
        .map_err(|error| anyhow::anyhow!("Could not obtain artifact ID randomness: {error}"))?;
    let suffix = random
        .iter()
        .map(|byte| format!("{byte:02x}"))
        .collect::<String>();
    Ok(format!("{date}-{millis:020}-{suffix}"))
}
...
/// UTF-8 convenience wrapper around [`create_new_file`].
pub fn create_new_text(path: impl AsRef<Path>, content: &str) -> Result<()> {
    create_new_file(path, content.as_bytes())
}
...
mod tests {
    ...
    #[test]
    fn artifact_instance_ids_are_sortable_and_collision_resistant() {
        let ids = (0..256)
            .map(|_| artifact_instance_id("2026-09-22"))
        ...
        assert!(ids
            .iter()
            .all(|id| id.starts_with("20260922-") && id.len() > "20260922-".len() + 20 + 32));
    }

    #[test]
    fn create_new_file_refuses_existing_target_without_overwrite() {
        let temp = tempdir().unwrap();
        let target = temp.path().join("value.txt");
        fs::write(&target, "before").unwrap();

        let error = create_new_text(&target, "after").unwrap_err();
    ...
    #[test]
    fn create_new_file_publishes_durable_content() {
        let temp = tempdir().unwrap();
        let target = temp.path().join("nested").join("value.txt");
