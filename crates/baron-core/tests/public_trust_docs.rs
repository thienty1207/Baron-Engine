...
fn readme_is_public_trust_landing_page_not_command_dump() {
    ...
    assert!(readme.contains("Current version: `3.1.3`"));
...
fn status_tracks_public_trust_phase() {
    ...
    assert!(status_md.contains("Stable source release: `v3.1.3`"));
    ...
    assert_eq!(status_json["stableRelease"], "3.1.3");
    assert_eq!(status_json["targetRelease"], "3.1.3");
    assert_eq!(
