...
fn supported_targets_have_stable_native_archive_names() {
    let names = SUPPORTED_RELEASE_TARGETS
        .iter()
        .map(|target| target.archive_name("3.1.0"))
        .collect::<Vec<_>>();

    assert_eq!(
        names,
        vec![
            "baron-v3.1.0-x86_64-pc-windows-msvc.zip",
            "baron-v3.1.0-x86_64-unknown-linux-gnu.tar.gz",
            "baron-v3.1.0-x86_64-apple-darwin.tar.gz",
            "baron-v3.1.0-aarch64-apple-darwin.tar.gz",
        ]
    );
}
...
fn manifest_and_checksum_output_are_deterministic_and_verifiable() {
    ...
    let windows = temp.path().join("baron-v3.1.0-x86_64-pc-windows-msvc.zip");
    let linux = temp
        .path()
        .join("baron-v3.1.0-x86_64-unknown-linux-gnu.tar.gz");
    ...
    assert!(checksums.contains("  baron-v3.1.0-x86_64-pc-windows-msvc.zip\n"));
    assert!(checksums.contains("  baron-v3.1.0-x86_64-unknown-linux-gnu.tar.gz\n"));
...
fn release_metadata_writer_requires_and_verifies_the_complete_platform_set() {
    ...
    for target in SUPPORTED_RELEASE_TARGETS {
        fs::write(
            temp.path().join(target.archive_name("3.1.0")),
            target.triple.as_bytes(),
        )
        .unwrap();
    }

    write_release_metadata(temp.path(), "3.1.0", "abc123").unwrap();
...
fn release_metadata_writer_rejects_a_missing_supported_target() {
    let temp = tempdir().unwrap();
    fs::write(
        temp.path().join("baron-v3.1.0-x86_64-pc-windows-msvc.zip"),
        b"windows",
    )
    .unwrap();

    let error = write_release_metadata(temp.path(), "3.1.0", "abc123")
        .unwrap_err()
        .to_string();
